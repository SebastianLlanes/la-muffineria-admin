...
export default function CalculadorPage() {
  ...
  const [hornadaActiva, setHornadaActiva] = useState(null) 
  const [hornadasVisible, setHornadasVisible] = useState(false)
    ...
    .reduce((acc, pedido) => {
      ...
      pedido.items?.forEach((it) => {
        ...
        acc[fecha].recetas[nombre] =
          (acc[fecha].recetas[nombre] || 0) + cantidad;
      });
      return acc;
    }, {});
  ...
  const factor =
    receta && receta.rendimiento > 0 ? cantidadNum / receta.rendimiento : 0;
  ...
  const costoIngredientes = ingredientesEscalados.reduce(
    (acc, ing) => acc + ing.costoReal,
    0,
  );
  ...
  const puntoEquilibrio =
    precioNum > costoPorUnidad && costoPorUnidad > 0
      ? Math.ceil(
          ((costoTotal / (precioNum - costoPorUnidad)) * costoPorUnidad) /
            costoPorUnidad,
        )
      : null;
  ...
  return (
    <div className={styles.page}>
      <div className={styles.topBar}>
        <div>
          ...
          <p className={styles.subtitle}>
            Proyectá una producción antes de arrancar
          </p>
        </div>
      ...
      {/* Hornadas pendientes */}
      {/* Hornadas pendientes — colapsable */}
      {hornadasOrdenadas.length > 0 && (
        <div className={styles.hornadasPanel}>
          <button
            className={styles.hornadasToggle}
            onClick={() => setHornadasVisible((v) => !v)}
          >
            <span>🔥 Hornadas pendientes ({hornadasOrdenadas.length})</span>
            <span>{hornadasVisible ? "▲" : "▼"}</span>
          </button>
                        ...
                        {Object.entries(data.recetas).map(([nombre, cant]) => {
                          ...
                          return (
                            <button
                              ...
                              onClick={() =>
                                recetaExiste &&
                                cargarDesdeHornada(fecha, nombre, cant)
                              }
                              disabled={!recetaExiste}
                              title={
                                !recetaExiste
                                  ? "Receta no encontrada en el sistema"
                                  : `Cargar: ${nombre} × ${cant} u.`
                              }
                            >
                              <span>{nombre}</span>
        ...
        <>
          ...
          <div className={styles.resumenGrid}>
            ...
            <div className={`${styles.resumenCard} ${styles.resumenDestacado}`}>
              ...
              <span className={styles.resumenValorGrande}>
          ...
          {precioNum > 0 && (
            <div className={styles.seccion}>
              ...
              <div className={styles.analisisGrid}>
                ...
                <div
                  className={`${styles.analisisCard} ${gananciaTotal >= 0 ? styles.positivo : styles.negativo}`}
                >
                ...
                <div
                  className={`${styles.analisisCard} ${margenPorcentaje >= 30 ? styles.positivo : margenPorcentaje >= 15 ? styles.neutro : styles.negativo}`}
                >
                ...
                <div className={styles.analisisCard}>
                  <span className={styles.analisisLabel}>
                    Ganancia por unidad
                  </span>
                  <span className={styles.analisisValor}>
              ...
              {precioNum <= costoPorUnidad && (
                <div className={styles.alerta}>
                  ⚠️ El precio de venta es menor al costo por unidad. Estás
                  perdiendo dinero.
                </div>
              )}
              ...
              {puntoEquilibrio !== null && precioNum > costoPorUnidad && (
                <div className={styles.equilibrio}>
                  ...
                  <strong>
                    vendiendo {cantidadNum} unidades a ${precioNum} cubrís
                    costos y ganás ${gananciaTotal.toFixed(2)}
                  </strong>
                </div>
              )}
            </div>
          ...
          <div className={styles.seccion}>
            <h3 className={styles.seccionTitle}>
              Precios sugeridos por margen objetivo
            </h3>
            <div className={styles.tableWrapper}>
              <table className={styles.table}>
                <thead>
                ...
                <tbody>
                  {margenesObjetivo.map((margen) => {
                    ...
                    const esActual =
                      precioNum > 0 && Math.abs(precio - precioNum) < 0.5;
                    return (
                      <tr
                        key={margen}
                      ...
                      >
                        <td>
                          <span
                            className={`${styles.margenBadge} ${styles[`m${margen}`]}`}
                          >
                            {margen}%
                          </span>
                        </td>
      ...
      {!recetaId && (
        <div className={styles.empty}>
          ...
          <p>
            Seleccioná una receta y una cantidad para ver la proyección de
            costos.
          </p>
        </div>
      )}
